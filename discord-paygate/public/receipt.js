...
function showConfirmed(plan, server) {
  ...
  callout.textContent = `${roles} was assigned automatically — your channels in ${server.name} are unlocked.`;
}
...
function showStillPending(server) {
  ...
  $('#r-callout').textContent =
    ...
    `provider confirms — no action needed. Check ${server.name} again in a minute.`;
}
...
async function main() {
  ...
  $('#open-discord-label').textContent = `Open ${server.name} on Discord`;
