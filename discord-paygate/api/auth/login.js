...
export const STORE_COOKIE = 'tl_checkout_store';
...
export default guard(function handler(req, res) {
  ...
  const storeSlug = /^[a-z0-9-]{1,40}$/.test(url.searchParams.get('store') ?? '') ? url.searchParams.get('store') : '';
  ...
  if (storeSlug) cookies.push(cookieHeader(STORE_COOKIE, storeSlug, { maxAge: 600, ...cookieAttrs() }));
